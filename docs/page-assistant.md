...
Browser sign-in errors carry a structured `ANAPLAN_BROWSER_LOGIN` code from the background request to the panel. Model authorization errors carry `ANAPLAN_LOGIN`, a validated Anaplan HTTPS URL and a sign-in code from the helper. Page verification, saved restoration and chat failures all reach the same Assistant recovery card, with separate Browser session and Model access statuses. Model failures no longer redirect to Admin; its connection card shares the challenge.

Browser expiry clears cached pages/definitions; either expiry invalidates the verification ticket. Retrying one connection cannot clear the other’s sign-in requirement. Model authorization success re-verifies page access, while failed retries keep recovery visible. Challenges stay in memory, are excluded from page snapshots and chat persistence, and clear after success. Failed saved-page restores retain prior context except when either sign-in is invalid; the expired ticket is then discarded while the saved chat remains readable. Ordinary unsupported-page errors still leave the picker available.
...
Load matching helper/extension files and reload the unpacked extension so 0.9.5 is active. The 0.9.4 → 0.9.5 runtime change is confined to the extension and requires no helper restart. Use a read-only test app with known totals.
...
8. Test expired browser/MCP sign-in, unavailable helper, missing page access and large catalogs. Either sign-in failure should show one recovery card with separately labeled Browser session and Model access statuses, with no page picker, starter prompts, idle composer or stale continuation notice. Test both sessions expiring and restoring each independently. Verify failed retry, replacement or missing authorization links, successful retry, no automatic Admin redirect or question resubmission, draft retention, access to History and Stop during a running answer. Record actionable recovery and latency.
