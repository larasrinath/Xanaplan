...
Current product behavior for version **0.9.5**. Start with the [setup guide](setup.md) for installation and sign-in.
...
A browser or model sign-in failure replaces the page picker, welcome prompts and idle composer with one **Connect to Anaplan** card. It labels the two connections **Browser session** (page and selections) and **Model access** (model data), each with its own status and recovery action. Browser recovery offers **Open Anaplan**; model recovery provides a validated authorization link and code. **Check connection** retries the relevant connection. Links open only when clicked in the Assistant; model authorization no longer switches the user to Admin. Admin retains the same authorization challenge.

Recovery remains visible through retries and temporary network failures until the affected connection succeeds. Browser expiry discards cached page definitions; either expiry invalidates the page ticket. Successful model reauthorization re-verifies page access. Continuation notices clear when their verified context becomes unavailable. Existing answers, unsent drafts and History are retained; a running answer and its Stop button remain available. A failed question returns to the draft and is never resubmitted automatically. Codes and links stay in memory only and clear on successful authorization.
...
**Follow current tab** matches the tab’s origin and app ID to exactly one enabled app at startup and while browsing. It reads the published board/worksheet and page/card selectors. Explicitly choosing an app or page pins the chat context until following resumes. Restored saved contexts also stay pinned. None of these choices navigates Anaplan. Unknown or ambiguous app matches require enablement or manual selection.

An empty chat greets the user according to local time. Starter questions come from verified card/module names and confirmed selections, including comparisons only between related cards. Suggestions use existing metadata and make no extra AI or Anaplan calls. Choosing one fills an editable draft. The shared theme uses near-white surfaces, blue actions, pale lavender highlights and small cream accents.

Questions appear in right-aligned bubbles; answers appear on the left with safe basic Markdown, source details and Copy answer. The composer stays beneath the independently scrolling conversation. Enter sends; Alt+Enter inserts a new line. The single send arrow becomes a square Stop icon while answering and returns when the request finishes, fails or is cancelled.

Answer activity has four animated stages driven by actual provider/MCP events, elapsed time and an expandable activity log. Heartbeats indicate a connected helper; stages do not advance just because time passes. A running question keeps its original thread and verified page/model/filter snapshot across tab, app, page and selection changes. The activity card identifies the original page when the current context differs. The finished answer stays visible with both continuation choices. Explicit Stop and closing the panel cancel the request.

**History** searches saved chats by question, app and page. Matching chats resume directly. **Continue on this page** uses fresh current context; **Use saved page & selections** verifies the original page and known selectors. Earlier answers retain their sources, and changed-context follow-ups receive a context marker. Unknown saved selectors remain unresolved and may require clarification for numeric reads. A failed restore retains the current page and recovery controls. See [saved conversations](chat-history.md).

Each question pins one verified source model from the enabled app. The helper verifies page sources and relevant formula dependencies, supplies scoped workspace/model IDs and rejects unrelated reads. Explicit question overrides affect only validated dimensions for that answer. Custom cards can provide underlying module evidence, but exact displayed totals require sufficient coverage and verified predicates. Cross-model comparisons require a future explicit flow. Live numerical correctness still needs checking against Anaplan. See [page coverage and acceptance](page-assistant.md).
