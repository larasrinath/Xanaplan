...
test('DOM end-to-end: navigate during an answer, retain its context, resume either page, and stop explicitly', async t => {
  ...
  const modelLogin = (details = {}) => new AppError('Synthetic model authorization required.', 401, { code: 'ANAPLAN_LOGIN', url: 'https://iam.anaplan.com/test-only', userCode: 'MODEL-TEST', ...details });
  let modelFailure = null, modelCheckDelay;
  const modelRead = mcp.read;
  mcp.read = (...args) => { if (modelFailure) throw modelFailure; return modelRead(...args); };
  mcp.discover = async () => {
    if (modelCheckDelay) await modelCheckDelay;
    if (modelFailure) throw modelFailure;
    return { items: [{ id: model.workspaceId, name: model.workspaceName }], incomplete: false };
  };
  ...
  assert.equal($('browser-connection-state').textContent, 'Sign-in needed');
  assert.equal($('model-connection-state').textContent, 'Not checked');
  modelFailure = modelLogin(); $('model-sign-in-retry').click();
  await until(() => $('model-connection-state').textContent === 'Sign-in needed' && !$('model-sign-in-retry').disabled);
  assert.equal($('admin-view').hidden, true, 'MCP sign-in stays in the Assistant');
  assert.equal($('browser-connection-state').textContent, 'Sign-in needed');
  assert.equal($('browser-sign-in-status').hidden, true, 'Do not show model errors in the browser connection');
  assert.equal($('model-sign-in-link').href, 'https://iam.anaplan.com/test-only');
  assert.equal($('model-sign-in-code').textContent, 'Sign-in code: MODEL-TEST');
  assert.equal($('auth-detail').textContent, $('model-sign-in-code').textContent, 'Admin shares the same authorization challenge');
  modelFailure = null; $('model-sign-in-retry').click();
  await until(() => $('model-connection-state').textContent === 'Connected' && !$('model-sign-in-retry').disabled);
  assert.equal($('browser-sign-in').hidden, false, 'Model authorization does not sign in the browser');
  assert.equal($('browser-connection-state').textContent, 'Sign-in needed');
  assert.equal($('model-sign-in-actions').hidden, true);
  assert.equal($('model-sign-in-code').textContent, ''); assert.equal($('model-sign-in-link').hasAttribute('href'), false);
  modelFailure = modelLogin();
  browserReadError = ''; $('browser-sign-in-retry').click();
  await until(() => $('model-connection-state').textContent === 'Sign-in needed' && !$('model-sign-in-retry').disabled);
  assert.equal($('browser-connection-state').textContent, 'Connected', 'Page verification forwards model authorization details');
  assert.equal($('browser-sign-in-actions').hidden, true); assert.equal($('question-form').hidden, true);
  assert.equal($('model-sign-in-link').hidden, false); assert.equal($('admin-view').hidden, true);
  const modelCheck = Promise.withResolvers(); modelCheckDelay = modelCheck.promise;
  modelFailure = new AppError('Synthetic model network failure.', 502);
  $('model-sign-in-retry').click();
  assert.equal($('model-sign-in-retry').disabled, true); assert.equal($('send').disabled, true);
  modelCheck.resolve(); modelCheckDelay = null;
  await until(() => !$('model-sign-in-retry').disabled && $('model-sign-in-status').textContent.includes('network failure'));
  assert.equal($('browser-sign-in').hidden, false); assert.equal($('admin-view').hidden, true);
  modelFailure = modelLogin({ url: 'https://untrusted.invalid/sign-in', userCode: '<img src=x onerror=alert(1)>' });
  $('model-sign-in-retry').click();
  await until(() => !$('model-sign-in-retry').disabled && $('model-sign-in-code').textContent.includes('<img'));
  assert.equal($('model-sign-in-code').children.length, 0);
  assert.equal($('model-sign-in-link').hidden, true); assert.equal($('model-sign-in-link').hasAttribute('href'), false);
  assert.equal($('auth-link').hasAttribute('href'), false);
  assert.match($('model-sign-in-status').textContent, /request a new sign-in link/);
  modelFailure = null; $('model-sign-in-retry').click();
  await until(() => !$('send').disabled);
  assert.equal($('model-sign-in-link').hasAttribute('href'), false); assert.equal($('model-sign-in-code').textContent, '');
  assert.equal($('auth').hidden, true); assert.equal($('auth-detail').textContent, '');
  ...
  // An expired model session during a read preserves history and the failed question as a draft.
  $('new-chat').click(); $('question').value = 'Model access baseline'; $('send').click();
  await until(() => !$('send').disabled && $('messages').querySelectorAll('.assistant').length === 1);
  pendingResolve = null; delayAnswer = true;
  $('question').value = 'Retry after authorization'; $('send').click();
  await until(() => pendingResolve);
  const expiredRequest = chatRequests.at(-1);
  modelFailure = modelLogin(); delayAnswer = false; pendingResolve();
  await until(() => !$('browser-sign-in').hidden && $('send').dataset.action === 'send');
  assert.equal($('admin-view').hidden, true); assert.equal($('question-form').hidden, true);
  assert.equal($('question').value, 'Retry after authorization');
  assert.equal($('messages').querySelectorAll('.assistant').length, 1);
  assert.equal(expiredRequest.signal.aborted, false);
  assert.equal(conversations.get(expiredRequest.body.conversationId).messages.length, 2);
  const requestCount = chatRequests.length;
  modelFailure = modelLogin({ userCode: 'REPLACEMENT-TEST' }); $('model-sign-in-retry').click();
  await until(() => !$('model-sign-in-retry').disabled && $('model-sign-in-code').textContent.includes('REPLACEMENT-TEST'));
  assert.equal($('question').value, 'Retry after authorization');
  $('admin-tab').click(); assert.equal($('admin-view').hidden, false);
  assert.equal($('auth-detail').textContent, 'Sign-in code: REPLACEMENT-TEST');
  modelFailure = null; $('connect-anaplan').click();
  await until(() => $('auth').hidden && !$('send').disabled);
  assert.equal($('admin-view').hidden, false, 'An explicit Admin check stays in Admin');
  $('assistant-tab').click();
  assert.equal($('browser-sign-in').hidden, true); assert.equal($('question').value, 'Retry after authorization');
  assert.equal(chatRequests.length, requestCount, 'Recovery never resubmits a question automatically');
  $('send').click();
  await until(() => !$('send').disabled && $('messages').querySelectorAll('.assistant').length === 2);
  assert.doesNotMatch(JSON.stringify(conversations.get(expiredRequest.body.conversationId)), /MODEL-TEST|REPLACEMENT-TEST|iam.anaplan.com/);
  assert.doesNotMatch(JSON.stringify(dom.window.localStorage), /MODEL-TEST|REPLACEMENT-TEST|iam.anaplan.com/);
