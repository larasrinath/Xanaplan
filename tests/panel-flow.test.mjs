...
test('DOM end-to-end: navigate during an answer, retain its context, resume either page, and stop explicitly', async t => {
  ...
  let browserReadError = 'ANAPLAN_BROWSER_LOGIN', browserReadDelay;
  ...
  install('chrome', { tabs: { query: async () => [{ id: 1, url: tab.url }], update: () => { navigationCalls++; }, create: () => { navigationCalls++; }, onUpdated: { addListener: listener => tabUpdates.add(listener), removeListener: listener => tabUpdates.delete(listener) } }, runtime: { sendMessage: async message => {
    ...
    if (browserReadDelay) await browserReadDelay;
    if (browserReadError) return { ok: false, code: browserReadError, error: browserReadError === 'ANAPLAN_BROWSER_LOGIN' ? 'Synthetic browser authorization required.' : 'Synthetic network unavailable.' };
  ...
  await until(() => !$('browser-sign-in').hidden && !$('browser-sign-in-retry').disabled);
  assert.equal($('live-context').hidden, true); assert.equal($('welcome').hidden, true);
  assert.equal($('question-form').hidden, true); assert.equal($('chat-privacy').hidden, true);
  assert.equal($('question-status').hidden, true); assert.equal($('send').disabled, true);
  assert.equal($('chat-page').options.length, 1, 'Do not retain a cached page list while signed out');
  assert.equal($('browser-sign-in-link').href, `${app.origin}/`);
  assert.equal($('browser-sign-in-status').hidden, true, 'The sign-in explanation appears once');
  assert.equal(navigationCalls, 0, 'Sign-in never opens or navigates a tab automatically');
  browserReadError = ''; $('browser-sign-in-retry').click();
  await until(() => !$('send').disabled);
  assert.equal($('browser-sign-in').hidden, true); assert.equal($('live-context').hidden, false);
  assert.equal($('question-form').hidden, false); assert.equal($('welcome').hidden, false);
  ...
  browserReadError = 'ANAPLAN_BROWSER_LOGIN'; $('refresh-page').click();
  await until(() => !$('browser-sign-in').hidden && !$('browser-sign-in-retry').disabled);
  assert.equal(runningRequest.signal.aborted, false, 'Browser sign-in loss must not cancel the current answer');
  assert.equal($('messages').querySelectorAll('.user').length, 2);
  assert.equal($('question-activity').hidden, false); assert.equal($('question-form').hidden, false);
  assert.equal($('send').dataset.action, 'stop'); assert.equal($('send').disabled, false);
  browserReadError = ''; $('browser-sign-in-retry').click();
  await until(() => $('browser-sign-in').hidden && !$('refresh-page').disabled);
  ...
  assert.match($('notice').textContent, /Continuing on Costs/);
  $('question').value = 'Keep my unsent draft';
  browserReadError = 'ANAPLAN_BROWSER_LOGIN'; $('refresh-page').click();
  await until(() => !$('browser-sign-in').hidden && !$('browser-sign-in-retry').disabled);
  assert.equal($('notice').hidden, true, 'The previous continuation notice must not survive lost page access');
  assert.equal($('messages').querySelectorAll('.assistant').length, 2, 'Existing answers remain readable while signed out');
  assert.equal($('question-form').hidden, true); assert.equal($('welcome').hidden, true);
  assert.equal($('question').value, 'Keep my unsent draft');
  browserReadError = 'ANAPLAN_BROWSER_UNAVAILABLE'; $('browser-sign-in-retry').click();
  await until(() => $('browser-sign-in-status').textContent === 'Synthetic network unavailable.');
  assert.equal($('browser-sign-in').hidden, false); assert.equal($('live-context').hidden, true);
  $('history-toggle').click(); await until(() => !$('history-refresh').disabled);
  assert.equal($('chat-history').hidden, false); assert.equal($('browser-sign-in').hidden, true);
  $('history-list').querySelector('.history-item').click(); await until(() => !$('saved-chat-heading').hidden);
  assert.equal($('browser-sign-in').hidden, false); assert.match($('messages').textContent, /Synthetic revenue/);
  assert.equal($('history-continue').disabled, true); assert.equal($('history-use-saved').disabled, true);
  $('history-back').click();
  browserReadError = '';
  const retry = Promise.withResolvers(); browserReadDelay = retry.promise;
  $('browser-sign-in-retry').click();
  assert.equal($('browser-sign-in-retry').disabled, true); assert.equal($('browser-sign-in').hidden, false);
  assert.equal($('live-context').hidden, true); assert.equal($('welcome').hidden, true);
  retry.resolve(); browserReadDelay = null;
  await until(() => !$('send').disabled);
  assert.equal($('browser-sign-in').hidden, true); assert.equal($('live-context').hidden, false);
  assert.equal($('question').value, 'Keep my unsent draft'); assert.equal($('notice').hidden, true);
  assert.equal($('messages').querySelectorAll('.assistant').length, 2);
  ...
  // Deleting the retained conversation while signed out must also clear its preview.
  $('question').value = 'Delete this saved chat'; $('send').click();
  await until(() => !$('send').disabled && $('messages').querySelectorAll('.assistant').length === 1);
  const deletedId = chatRequests.at(-1).body.conversationId;
  browserReadError = 'ANAPLAN_BROWSER_LOGIN'; $('refresh-page').click();
  await until(() => !$('browser-sign-in').hidden && !$('browser-sign-in-retry').disabled);
  assert.equal($('messages').querySelectorAll('.assistant').length, 1);
  $('history-toggle').click(); await until(() => !$('history-refresh').disabled);
  install('confirm', () => true);
  $('history-list').querySelector('.history-delete').click();
  await until(() => !$('history-list').textContent.includes('Delete this saved chat'));
  $('history-back').click();
  assert.equal($('messages').children.length, 0);
  assert.throws(() => conversations.get(deletedId), /not found/);
  assert.equal($('browser-sign-in').hidden, false);
});
